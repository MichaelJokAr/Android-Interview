...
### put逻辑
    ...
    - 对key值求Hash值，然后计算下标
    - 如果没有碰撞直接放入桶中
    - 如果碰撞了，且key不相同则以链表的方式链接到后面
    - hash碰撞了，且key equals比较相同则替换旧值
    - 如果链表超过阀值（8）就把链表转化为红黑树
    - 如果链表长度<=6就把红黑树转回链表
    - 如果桶满了（容量16*加载因子0.75）就需要resize（扩容2后重拍）
...
## **java new一个对象的过程**
- 当虚拟机遇到一条new指令时候，首先去检查这个指令的参数是否能 在常量池中能否定位到一个类的符号引用 （即类的带路径全名），并且检查这个符号引用代表的类是否已被加载、解析和初始化过，即验证是否是第一次使用该类。如果没有（不是第一次使用），那必须先执行相应的类加载过程（class.forname()）。
...
> [https://www.cnblogs.com/gjmhome/p/11401397.html](https://www.cnblogs.com/gjmhome/p/11401397.html)
...
- volatile的作用是禁止重排序。
    ...
    synchronized 禁止重排序是通过内存屏障实现的, 简单来说, 内存屏障只保证指令不会越过该屏障, 而synchronized块内部的指令仍然有可能发生重排序，volatile可以禁止指令重排序从而避免这个问题。
